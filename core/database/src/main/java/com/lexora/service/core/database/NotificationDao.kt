...
interface NotificationDao {
    ...
    @Query("SELECT * FROM service_notifications WHERE id = :id LIMIT 1")
    suspend fun notification(id: String): ServiceNotificationEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsert(value: ServiceNotificationEntity)

    @Query("UPDATE service_notifications SET readAtEpochMs = :readAtEpochMs, syncState = :syncState, updatedAtEpochMs = :updatedAtEpochMs WHERE id = :id")
    ...
    @Query("UPDATE service_notifications SET archived = :archived, syncState = :syncState, updatedAtEpochMs = :updatedAtEpochMs WHERE id = :id")
