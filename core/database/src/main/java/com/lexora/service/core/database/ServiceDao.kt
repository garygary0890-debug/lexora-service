...
interface ServiceDao {
    ...
    @Query("UPDATE sync_operations SET status = 'IN_PROGRESS', attemptCount = attemptCount + 1, updatedAtEpochMs = :updatedAt WHERE id = :id AND organizationId = (SELECT id FROM organizations WHERE isActive = 1 LIMIT 1) AND status IN ('PENDING','RETRY_WAIT')") suspend fun markSyncOperationInProgress(id: String, updatedAt: Long): Int
    @Query("UPDATE sync_operations SET status = 'SUCCEEDED', lastError = NULL, nextAttemptAtEpochMs = NULL, updatedAtEpochMs = :updatedAt WHERE id = :id AND organizationId = (SELECT id FROM organizations WHERE isActive = 1 LIMIT 1)") suspend fun markSyncOperationSucceeded(id: String, updatedAt: Long)
    @Query("UPDATE sync_operations SET status = 'RETRY_WAIT', lastError = :error, nextAttemptAtEpochMs = :nextAttemptAt, updatedAtEpochMs = :updatedAt WHERE id = :id AND organizationId = (SELECT id FROM organizations WHERE isActive = 1 LIMIT 1)") suspend fun markSyncOperationRetry(id: String, error: String?, nextAttemptAt: Long, updatedAt: Long)
    @Query("UPDATE sync_operations SET status = 'FAILED', lastError = :error, nextAttemptAtEpochMs = NULL, updatedAtEpochMs = :updatedAt WHERE id = :id AND organizationId = (SELECT id FROM organizations WHERE isActive = 1 LIMIT 1)") suspend fun markSyncOperationFailed(id: String, error: String?, updatedAt: Long)
    @Query("UPDATE sync_operations SET status = 'CONFLICT', lastError = :error, nextAttemptAtEpochMs = NULL, updatedAtEpochMs = :updatedAt WHERE id = :id AND organizationId = (SELECT id FROM organizations WHERE isActive = 1 LIMIT 1)") suspend fun markSyncOperationConflict(id: String, error: String?, updatedAt: Long)
    ...
    @Query("UPDATE sync_conflicts SET resolution = :resolution, resolvedAtEpochMs = :resolvedAt WHERE id = :id AND organizationId = (SELECT id FROM organizations WHERE isActive = 1 LIMIT 1)") suspend fun resolveSyncConflict(id: String, resolution: String, resolvedAt: Long)
}
