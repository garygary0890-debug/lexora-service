...
interface ServiceDao {
    ...
    @Query("UPDATE service_requests SET plannedAtEpochMs = :startAt, plannedEndEpochMs = :endAt, syncState = :syncState, updatedAtEpochMs = :updatedAt WHERE id = :id AND organizationId = :organizationId") suspend fun rescheduleServiceRequest(id: String, organizationId: String, startAt: Long, endAt: Long, syncState: String, updatedAt: Long)
    @Query("UPDATE service_requests SET firstReactionAtEpochMs = COALESCE(firstReactionAtEpochMs, :reactedAt), syncState = :syncState, updatedAtEpochMs = :updatedAt WHERE id = :id AND organizationId = :organizationId") suspend fun markRequestReaction(id: String, organizationId: String, reactedAt: Long, syncState: String, updatedAt: Long)
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insertRequestChangeHistory(value: RequestChangeHistoryEntity)
    @Query("SELECT * FROM request_change_history WHERE requestId = :requestId AND requestId IN (SELECT id FROM service_requests WHERE organizationId = :organizationId) ORDER BY changedAtEpochMs DESC") suspend fun requestChangeHistory(organizationId: String, requestId: String): List<RequestChangeHistoryEntity>
    ...
    @Query("UPDATE service_visits SET plannedStartEpochMs = :startAt, plannedEndEpochMs = :endAt, syncState = :syncState, updatedAtEpochMs = :updatedAt WHERE requestId = :requestId AND organizationId = :organizationId AND status = 'PLANNED'") suspend fun reschedulePlannedVisitsForRequest(requestId: String, organizationId: String, startAt: Long?, endAt: Long?, syncState: String, updatedAt: Long)
